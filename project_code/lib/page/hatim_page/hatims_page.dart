...
class _HatimsPageState extends State<HatimsPage> {
  ...
  // Helper to check if user is the creator-admin of the group
  bool isOwnerAdmin(GroupModel group, String currentUserId) {
    final userController = Provider.of<UserController>(context, listen: false);
    return userController.userModel?.isAdmin == true &&
        group.adminId != null &&
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<GroupModel?>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            final bool ownerAdmin = isOwnerAdmin(group, userID);
                    ...
                    ownerAdmin
                        ...
                        : lang.youAreRightNowAtThatXHatimAndYouNeedToReadThisXChapter!(
                            hatim: group
                                ...
                                .toString(),
                            chapter: group.getCurrentHatimChapterOfUser(userID),
                          ),
                  ...
                  child: Row(
                    ...
                    children: [
                      Expanded(
                        ...
                        child: Text(
                          ownerAdmin
                              ? lang.toSeeMoreDetailsAboutTheHatimPressOnTheHatim!
                              : lang.ifYouCompleteTheHatimYouNeedToPressOnTheHatimToUpdateYourHatimRound!,
                    ...
                    child: Padding(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isCurrentIndex = ownerAdmin
                              ? group.getCurrentHatim() == index + 1
                              : group.getCurrentHatimOfUser(userID) ==
                                    index + 1;

                          final didUserCompleteHatim = ownerAdmin
                              ? hatim.isAllUserCompleted(group.usersID.length)
                              : hatim.isHatimCompleted(userID);

                          // each hatim end date
                          final endDate = hatim.getEndDate(
                            group.startDate!,
                            group.dateType,
                          );
                          final endDateString = ownerAdmin
                          ...
                          final startHatimDateMessage =
                              '${lang.theHatimWillStartAtThatDate}: ${hatim.getStartDate(group.startDate!, group.dateType).day}/${hatim.getStartDate(group.startDate!, group.dateType).month}/${hatim.getStartDate(group.startDate!, group.dateType).year}';
                                ...
                                ///
                                 if (didUserCompleteHatim) {
                                  ...
                                  if (result != null && result) {
                                    ...
                                    double scoreReward = 0.2;
                                    int chapterReward = 1;
                                    int hatimReward = 0;

                                    // Check if this action completed the entire Hatim program for the user
                                    if (group.isUserFinishedAllRounds(userID)) {
                                      scoreReward += 1.0;
                                      hatimReward += 1;
                                    }
                                    ...
                                    userController.updateUserScore(
                                      ...
                                      chapterDelta: chapterReward,
                                      hatimDelta: hatimReward,
                                    );
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        lang.youNeedToCompleteThisHatimToBeAbleToGoToTheNextHatim!,
                                      ),
                                    ),
                                  );
                                }
                                    ...
                                    children: [
                                      ...
                                      ownerAdmin
                                          ? ProgressIndicatorWithNumber(
                                              ...
                                              totalValue: group.usersID.length,
                                            )
                                          : Icon(
                                              didUserCompleteHatim
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          var group = await groupController.getGroupByID(
        ...
        label: Text(
          UserController.userID() == '5065576580'
              ? 'Ali İhsan ❤️'
              : lang.myCurrentHatim!,
        ),
